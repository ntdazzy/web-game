...
$(document).ready(function () {
    ...
    if (btnTurnIn && btnTurnOut && btnTurnTop && rightMenu) {
    ...
    if(status == 1 && data){
        for (const [key, value] of Object.entries(data)) {
            html += '<tr>' +
                        ...
                        '<td><span class="minitit-head">Gói đã mua</span><span class="minitit-body">'+(function(){
                            var store = typeof packages !== 'undefined' ? packages : (window.packages || {});
                            var pkg = store['package-' + value.item];
                            return pkg && (pkg.name || pkg['name']) ? (pkg.name || pkg['name']) : '';
                        })()+'</span></td>' +
                    '</tr>';
        }
    }else{
