...
const selectedMethod = computed(() =>
    props.paymentMethods.find((method) => method.id === selectedMethodId.value) ?? null,
);

const selectedPackage = computed(
    () => props.packages.find((item) => item.amount === selectedAmount.value) ?? null,
);
...
const confirmTopUp = async () => {
    ...
    if (selectedPackage.value) {
        summaryLines.push(
            `• Gói nạp: GEM ${formattedNumber(selectedPackage.value.gems)} (bonus ${formattedNumber(
                selectedPackage.value.bonus_gems ?? 0,
            )})`,
        );
        summaryLines.push(`• Giá trị: ${formattedCurrency(selectedPackage.value.amount)}`);
    }
                        ...
                        <div class="payment w-100">
                            <ul class="payment-tab w-100">
                                ...
                                >
                                    <component
                                        :is="tab.href ? Link : 'a'"
                                        v-bind="tab.href ? { href: tab.href } : { href: 'javascript:void(0)' }"
                                    >
                                        {{ tab.label }}
                                    </component>
                                </li>
                            ...
                            <div class="payment-userinfo w-100">
                                <ul>
                                    <li>
                                        <span class="uname-label">Tài khoản: </span>
                                        <b class="display-name">{{ props.user.name }}</b>
                                    </li>
                                    <li>
                                        <span class="gem-label color-blue">GEM</span>:
                                        <b class="display-balance">
                                            {{ formattedNumber(props.user.gem_balance) }}
                                        </b>
                                    </li>
                                </ul>
                            </div>
                            ...
                            <div class="link-to-history w-100 text-center">
                                <Link :href="historyRoute" class="login-required" data-redirect="qua-nap-web.html">
                                    Lịch sử nạp
                                </Link>
                            </div>
                            ...
                            <div class="item-list w-100">
                                ...
                                <div class="item-list-payment-type">
                                    ...
                                    >
                                        <input type="hidden" name="ftype" :value="method.id" />
                                        <div
                                            class="img"
                                        ...
                                        ></div>
                                        <h6>{{ method.label }}</h6>
                                        <span v-if="method.ribbon" class="ribbon">{{ method.ribbon }}</span>
                                    </a>
                                </div>
                            </div>
                            ...
                            <div class="item-list w-100">
                                ...
                                <div class="item-list-slick">
                                    <div v-for="pkg in packages" :key="pkg.amount" class="item">
                                        <a
                                            href="javascript:void(0)"
                                            :class="{ active: pkg.amount === selectedAmount }"
                                            @click.prevent="selectPackage(pkg.amount)"
                                        >
                                            <input type="hidden" name="famount" :value="pkg.amount" />
                                            <span class="top">
                                                GEM <b>{{ formattedNumber(pkg.gems) }}</b>
                                            </span>
                                            <span class="middle">
                                                <b>+ {{ formattedNumber(pkg.bonus_gems ?? 0) }}</b>
                                                BONUS
                                            </span>
                                            <span class="bot">
